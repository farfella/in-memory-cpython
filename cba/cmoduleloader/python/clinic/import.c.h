...
// CBA
PyDoc_STRVAR(_imp_create_dynamic_inmemory__doc__,
"create_dynamic_inmemory($module, spec, file=<unrepresentable>, /)\n"
"--\n"
"\n"
"Create an extension module in memory.");

#define _IMP_CREATE_DYNAMIC_INMEMORY_METHODDEF    \
    {"create_dynamic_inmemory", (PyCFunction)(void(*)(void))_imp_create_dynamic_inmemory, METH_FASTCALL, _imp_create_dynamic_inmemory__doc__},

static PyObject*
_imp_create_dynamic_inmemory_impl(PyObject* module, PyObject* spec, PyObject* data);

static PyObject*
_imp_create_dynamic_inmemory(PyObject* module, PyObject* const* args, Py_ssize_t nargs)
{
    PyObject* return_value = NULL;
    PyObject* spec;
    PyObject* data;

    if (!_PyArg_CheckPositional("create_dynamic_inmemory", nargs, 2, 2)) {
        goto exit;
    }
    spec = args[0];
    data = args[1];

    return_value = _imp_create_dynamic_inmemory_impl(module, spec, data);
