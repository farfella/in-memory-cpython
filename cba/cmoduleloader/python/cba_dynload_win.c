...
static char *CBA_GetPythonImport (PLOADEDMODULE pModule)
{
    ...
    dllbase = (unsigned char *)pModule->pCodeBase;
...
dl_funcptr _PyImport_CBA_FindSharedFuncptrWindows(const char *prefix,
                                                     ...
                                                     Py_ssize_t size)
{
...

    {
        ...
        pModule = LoadModuleFromMemory(hModule, (DWORD)size);
        ...
        if (pModule==NULL){
            ...
            if (message != NULL) {
                ...
                PyErr_SetImportError(message, shortname_obj, Py_None);
            ...
            import_python = CBA_GetPythonImport(pModule);
        ...
        Py_BEGIN_ALLOW_THREADS
        p = _GetProcAddress(pModule, funcname);
        Py_END_ALLOW_THREADS
    }
