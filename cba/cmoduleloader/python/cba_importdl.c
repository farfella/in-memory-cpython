...
#include "importdl.h"
#include "cba_importdl.h"

#ifdef MS_WINDOWS
extern dl_funcptr _PyImport_CBA_FindSharedFuncptrWindows(const char *prefix,
                                                     const char *shortname,
                                                     void * hModule, Py_ssize_t size);
#else
extern dl_funcptr _PyImport_CBA_FindSharedFuncptr(const char *prefix,
                                              const char *shortname,
                                              const char *pathname, void * hModule);
#endif
...
PyObject *
_PyImport_CBA_LoadDynamicModuleWithSpec(PyObject *spec, PyObject * data)
{
#ifndef MS_WINDOWS
    PyObject *pathbytes = NULL;
#endif
    PyObject *name_unicode = NULL, *name = NULL, *m = NULL;
    ...
    if (PySys_Audit("import", "OOOOO", name_unicode, Py_None,
                    Py_None, Py_None, Py_None) < 0) {
        return NULL;
    }

    if (PyBytes_AsStringAndSize(data, &module_data, &module_size))
    {
        goto error;
    }

#ifdef MS_WINDOWS
    exportfunc = _PyImport_CBA_FindSharedFuncptrWindows(hook_prefix, name_buf, module_data, module_size);
#else
    ...
    if (exportfunc == NULL) {
        if (!PyErr_Occurred()) {
            ...
            if (msg == NULL)
                goto error;
            PyErr_SetImportError(msg, name_unicode, Py_None);
            Py_DECREF(msg);
        }
        goto error;
    }
    ...
    if (PyModule_AddObject(m, "__file__", Py_None) < 0)
        PyErr_Clear(); /* Not important enough to report */

    PyObject *modules = PyImport_GetModuleDict();
    if (_PyImport_FixupExtensionObject(m, name_unicode, Py_None, modules) < 0)
        goto error;
