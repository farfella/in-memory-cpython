...
CDataType_in_dll(PyObject *type, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
        // CBA
        if (handle == PyWin_DLLhModule)
        {
            address = (void *)_GetProcAddress(PyWin_DLL, name);
        }
        else
        {
...
static PPROC FindAddress(void *handle, const char *name, PyObject *type)
{
    PPROC address;
    // CBA
    if (NULL == PyWin_DLLhModule) {
        PyGILState_STATE gstate = PyGILState_Ensure();
        PyObject * dllhandle = PySys_GetObject("dllhandle");
        if (PyLong_Check(dllhandle)) {
            PyWin_DLLhModule = PyLong_AsVoidPtr(dllhandle);
        }

        PyGILState_Release(gstate);
        PyWin_DLL = _LoadModuleFromInstance(PyWin_DLLhModule);
    }
    // CBA
    ...
    */
    Py_BEGIN_ALLOW_THREADS
    // CBA
    if (handle == PyWin_DLLhModule && NULL != PyWin_DLL) {
        address = (PPROC)_GetProcAddress(PyWin_DLL, name);
    }
    else {
    ...
    for (i = 0; i < 32; ++i) {
        ...
        // CBA
            if (handle == PyWin_DLLhModule)
            {
                address = (PPROC)_GetProcAddress(PyWin_DLL, mangled_name);
            }
            else
            {
